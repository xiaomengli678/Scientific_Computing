function [B1,B2]=RK4other(t,Gama2,B_leader,Binital_other,Rho,Delta,Kappa)
...
f1 = @(i,Bx) Gama2*(B_leader(1,i)-Bx) + Kappa*(mean([B_leader(1,i);B1(:,i)])-Bx) + Rho*(Bx-(sum(B1(:,i))-Bx)/(Bx-(sum(B1(:,i))-Bx)^2 + Delta));
f2 = @(i,By) Gama2*(B_leader(2,i)-By) + Kappa*(mean([B_leader(2,i);B2(:,i)])-By) + Rho*(By-(sum(B2(:,i))-By)/(By-(sum(B2(:,i))-By)^2 + Delta));
for j= 1:n
    for i = 1:size(B1,1)
   K1x = f1(j,B1(i,j));
   K1y = f2(j,B2(i,j));
