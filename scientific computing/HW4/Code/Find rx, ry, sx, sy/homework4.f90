program hwk4
  ...
  real(kind = 8), dimension(:,:), allocatable :: rx,ry,sx,sy,tmp1,tmp2,tmp3
  ...
  allocate(rx(0:nr,0:ns),sx(0:nr,0:ns),ry(0:nr,0:ns),sy(0:nr,0:ns))
  ...
  do j = 0,ns
     do i = 0,nr
        ...
        tmp3 = 1/( tmp1(1,1)*tmp1(2,2) -  tmp1(1,2)* tmp1(2,1))*tmp2
        rx(i,j) =  tmp3(1,1)
        ry(i,j) =  tmp3(2,1)
        sx(i,j) =  tmp3(1,2)
        sy(i,j) =  tmp3(2,2)
